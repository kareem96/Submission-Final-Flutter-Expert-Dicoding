...
void main(){
  ...
  blocTest<TvRecommendationBloc, TvRecommendationState>(
    ...
    build: () {
      when(mockGetRecommendationsTv.execute(testId)).thenAnswer((_) async => Right(testTVShowList));
  ...
  blocTest<TvRecommendationBloc, TvRecommendationState>('should emit Loading state and then Error state when data failed to fetch', build: () {
  ...
  blocTest<TvRecommendationBloc, TvRecommendationState>(
    ...
    build: () {
      when(mockGetRecommendationsTv.execute(testId)).thenAnswer((_) async => const Right([]));
      return tvRecommendationBloc;
    },
    act: (bloc) => bloc.add(OnTvRecommendation(testId)),
    expect: () => [
